...
public class HttpAPIRequester {
	...
	/**
	 * HTTP api 调用方法，回应将再responser里面处理 execute(这里用一句话描述这个方法的作用） （这里描述这个方法是用条件 -
	 * 可选）
	 * 
	 * @param dtype
	 *            返回数据的 data节点数据泛型
	 * @param dltype
	 *            返回数据的 dataList节点数据泛型
	 * @param url
	 *            void
	 * @exception
	 * @since 1.0.0
	...
	public void execute(Type dtype, Type dltype, final String url) {
		...
		executor.execute(new Runnable() {
			...
			public void run() {
				...
				try {
					String dataString = httpPost(url,
							responser.getRequestParams());
					...
					if (json.containsKey("dataList") && dataListType != null) {
						dataString = json.getJSONArray("dataList")
								.toJSONString();
						data.put("list",
								JSON.parseObject(dataString, dataListType));
					}

					if (json.containsKey("page")
							&& json.getJSONObject("page") != null) {
						dataString = json.getJSONObject("page").toJSONString();
						data.put("page",
								JSON.parseObject(dataString, Page.class));
					}
	...
	public static String httpPost(String url, Map<String, ?> map)
			throws Exception {
	...
	Handler handler = new Handler() {
		public void handleMessage(Message message) {
			...
			case 0:
				...
				responser.onSuccess(data.get("data"), list, page, String
						.valueOf(data.get("code")), data.get("url").toString());

				break;
			case 1:
				responser.onFailed((Exception) message.getData().get(
						"exception"));
				break;
			}
